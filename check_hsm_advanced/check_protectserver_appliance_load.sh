...
# TODO IMPORTANT: to avoid locking SSH access to the appliance, if a response of "Account locked due to X failed logins" is received, make all the following attempts to execute this plugin fail prematurely, maybe by writing a lock file that should be removed manually, e.g. /tmp/check_protectserver_appliance_load_locked_<HSM_IP>.
status_cpu_output=$(ssh -S "~/.ssh/S.%r@%h:%p" pseoperator@$hsm_ip "status cpu")
